...
class ArchiveStore:
    ...
    def _collect_wal(self) -> None:
        """Drop WAL files below the retention horizon."""
    ...
    def rebuild_segment(self, seg_id: str) -> dict:
        """Rebuild a quarantined segment from its retained WAL file."""
        with self._lock:
            meta = self._seg_by_id.get(seg_id)
            if meta is None:
                raise NotFound(f"segment {seg_id} not found")
            if meta["status"] != "quarantined":
                return dict(meta)
            wal_path = os.path.join(self.wal_dir, f"{meta['first_offset']:020d}.wal")
            if not os.path.exists(wal_path):
                raise WalCoverageGone(seg_id, meta["last_offset"] + 1)
            records = [
                r for r in (json.loads(p) for p in walmod.read_all_payloads(wal_path))
                if meta["first_offset"] <= r["offset"] <= meta["last_offset"]
            ]
